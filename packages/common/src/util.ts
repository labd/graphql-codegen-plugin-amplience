...
export const ifValue = <T, S>(
  value: T | undefined,
  ifValue: (v: T) => S,
): S | undefined => (value === undefined ? undefined : ifValue(value));

export const ifNotEmpty = <T, S>(
  items: T[],
  callback: (items: T[]) => S,
): S | undefined => (items.length ? callback(items) : undefined);
