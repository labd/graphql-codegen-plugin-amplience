...
import { snakeCase, kebabCase, capitalCase } from "change-case";
import {
...
import { VisualizationType } from "./config.js";
  ...
  ({
    ...
    (node: ObjectTypeDefinitionNode) => {
      ...
      const schema = tfg.resource("amplience_content_type_schema", name, {
        body: fn(
          "file",
          `\${path.module}/schemas/${kebabCase(node.name.value)}${schemaSuffix ? "-" + schemaSuffix : ""
          }.json`,
        ),
