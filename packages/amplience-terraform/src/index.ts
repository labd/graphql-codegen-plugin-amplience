...
import {
  ...
  findDirectiveValue,
  typeUri
} from './lib/amplience-schema-transformers'
...
// export { addToSchema } from '@amplience-codegen/graphql'
  ...
  visit(astNode, {
    leave: {
      ObjectTypeDefinition: node => {
        const directive = findDirective(node, 'amplience')
        if (!directive) return null

        const name = snakeCase(node.name.value)

        const isSlot =
          findDirectiveValue<EnumValueNode>(directive, 'validationLevel')
            ?.value === 'SLOT'
        ...
        const shouldVisualize = findDirectiveValue<BooleanValueNode>(
          directive,
          'visualizations'
        )?.value
        ...
        const repositoryName = findDirectiveValue<StringValueNode>(
          directive,
          'repository'
        )?.value
